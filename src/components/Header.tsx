    ...
    <header className="fixed top-0 left-0 right-0 bg-[#1E1E3F]/80 backdrop-blur-md z-50" dir={currentLang === "en" ? "ltr" : "rtl"}>
      <nav className="max-w-4xl mx-auto px-4">
        <div className="flex justify-between items-center h-16">
          <h1 className={`text-3xl font-bold text-[#FF00FF] ${jaro.className}`}>Yacinoxo</h1>
          <Link href={currentLang === "en" ? "/ar" : "/"}
            onClick={toggleLanguage}
            className={`${currentLang === 'en' ? lalezar.className : jaro.className} px-4 py-2 rounded bg-[#2D2B55] text-white hover:bg-[#373561] transition-colors`}
          >
